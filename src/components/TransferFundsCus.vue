...
async function fetchAccounts() {
  try {
    const response = await axios.get(
      API_ENDPOINTS.accountsById(authStore.user.id)
    );
  ...
  try {
    ...
    if (finalTransaction.status === "SUCCEEDED") {
      toastRef.value.setToast(
        "Transaction succeeded, funds transferred",
        "success"
      );
      await fetchAccounts();
      resetForm();
      ...
      toastRef.value.setToast(
        finalTransaction.failureReason ||
    ...
    <form @submit.prevent="onSubmit" class="d-flex flex-column gap-2">
      <AccountDropdown
        ...
        @update:selectedAccount="
          (account) =>
            setFieldValue('fromAccountIban', account ? account.iban : '')
        "
      />
      <AccountDropdown
        :accounts="accounts"
        ...
        @update:selectedAccount="
          (account) =>
            setFieldValue('toAccountIban', account ? account.iban : '')
        "
      />
